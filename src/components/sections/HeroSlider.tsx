/**
 * HeroSlider — full image slider version of the Hero section.
 * Ready to activate: swap Hero → HeroSlider in src/app/page.tsx.
 * Images must be optimised for all breakpoints before going live.
 *
 * Slider images: public/images/slider images/slider-{1,2,3}.jpg
 */
'use client';
                ...
                <div className="max-w-[1320px] mx-auto">
                    <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 lg:gap-10">
                        ...
                        <div className="flex flex-col gap-4 md:gap-5 lg:gap-6">
                            ...
                            <motion.div custom={0} variants={fadeUp} initial="hidden" animate="visible" className="flex items-center gap-2">
                                <span className="w-[6px] h-[6px] rounded-full bg-[#d4d4d4] shrink-0" />
                                <p className="font-medium tracking-[0.18em] text-[#d4d4d4] uppercase"
                                    style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(0.7rem, 1.1vw, 0.8rem)' }}>
                            ...
                            <div className="flex flex-col gap-3 md:gap-4">
                                <motion.h1 custom={1} variants={fadeUp} initial="hidden" animate="visible"
                                    style={{ fontFamily: 'var(--font-editorial)', fontWeight: 400, fontStyle: 'normal', color: '#ffffff', lineHeight: 1.02, textWrap: 'nowrap' as const, fontSize: 'clamp(1.9rem, 5.5svh, 3.8rem)' }}>
                                ...
                                <motion.p custom={2} variants={fadeUp} initial="hidden" animate="visible"
                                    ...
                                    style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(0.9rem, 1.3vw, 1.05rem)', maxWidth: '38ch' }}>
                                    Uniting Africa to grow resilient, trusted organic food systems
                                </motion.p>
                            </div>

                            <motion.div custom={3} variants={fadeUp} initial="hidden" animate="visible" className="flex flex-col gap-5 items-start">
                                <Link href="/programme" className="hero-cta has-slide">
                                    <span className="slide-wrap font-medium text-[0.9rem] whitespace-nowrap" style={{ fontFamily: 'var(--font-display)', color: 'inherit' }}>
                                ...
                                <div className="flex items-center gap-[6px]">
                                    {slides.map((_, i) => (
                                        <button
                                            ...
                                            style={{ width: i === current ? '52px' : '20px', background: 'rgba(255,255,255,0.28)', transition: 'width 0.35s ease' }}
                                        >
                                            {i === current && (
                                                <span
                                                    key={`progress-${current}`}
                                                    className="absolute inset-y-0 left-0 w-full bg-white rounded-full"
                                                    style={{ transformOrigin: 'left center', animation: `hero-progress ${SLIDE_DURATION}ms linear forwards` }}
                                                />
                                            )}
                                        </button>
                                    ))}
                        ...
                        <motion.div custom={4} variants={fadeUp} initial="hidden" animate="visible" className="hidden lg:block shrink-0 w-[270px]">
                            <div className="news-panel">
                                <div className="news-panel-header">
                                    <div className="flex items-center gap-2">
                                        <span className="news-dot" />
                                        <span className="font-semibold tracking-[0.15em] text-white uppercase" style={{ fontFamily: 'var(--font-display)', fontSize: '0.68rem' }}>
                                            News Updates
                                        </span>
                                    </div>
                                    <Link href="/news" aria-label="Go to all news" className="news-arrow-link" style={{ textDecoration: 'none' }}>
                                        <ArrowNE size={13} />
                                    </Link>
                                </div>
                                ...
                                <div className="news-panel-body">
                                    <Link href="/news" className="news-item group" style={{ textDecoration: 'none' }}>
                                        <p className="leading-snug text-white/90 group-hover:text-white transition-colors" style={{ fontFamily: 'var(--font-display)', fontSize: '0.82rem', maxWidth: 'none' }}>
